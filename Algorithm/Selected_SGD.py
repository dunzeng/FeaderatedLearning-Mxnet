...
class SSGD_Server_Tool:
    # Selected SGD algorithm
    # From paper Privacy-Preserving Deep Learning
    # class for Server
    def __init__(self,network,theat_d,lambda_,tao):
        self.stat = []
        self.max_stat = 0
        self.theat_d = theat_d # 模型参数选择比例
        self.lambda_ = lambda_
        self.tao = tao
        self.__init_stat(network)

    def __init_stat(self,network):
        # 根据network结构初始化stat列表
        for layer in network:
            try:
                shape = layer.weight.data().shape
                self.stat.append(np.zeros(shape = shape))
            except:
                pass
    ...
    def get_model_selected(self,origin_net,save_path,threshold=0):
        # 根据stat列表
...
class SSGD_Client_Tool:
    ...
    def load_selected_model(self,network,selected_model_path):
        ...
        for layer in network:
            try:
                ...
                layer_np = layer.weight.data()[:].asnumpy()
                layer_np[data_np!=0] = 0
                layer_np += data_np
                layer_nd = nd.array(layer_np)
                layer.weight.set_data(layer_nd)
            except:
                print("no weights in ",layer)